...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        """
        # retorna None caso não exista essa key no dict
        q_value = self.q.get((tuple(state), action)) 

        return q_value if q_value != None else 0
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if best_q_value == 0:
            ...
            # retorna o primeiro valor do gerador
            return next(action for (s, action) in self.q if self.q[(s, action)] == best_q_value)
